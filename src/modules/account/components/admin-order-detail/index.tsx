...
export default function AdminOrderDetail({ order }: { order: any }) {
  ...
  const itemsToFulfill = useMemo(
    () =>
      (order.items || []).filter(
        (i: any) => (i.detail?.fulfilled_quantity || 0) < i.quantity
      ),
    [order.items]
  )
    ...
    ...(order.payment_collections || [])
      .flatMap((c: any) =>
        (c.payments || []).map((p: any) =>
          p.captured_at
            ? { label: t("timelinePaymentCaptured"), at: p.captured_at }
            : null
        )
      )
      .filter(Boolean),
    ...(order.fulfillments || []).map((f: any) => ({
      label: t("timelineFulfilled"),
      at: f.created_at,
    })),
    ...(order.returns || []).map((r: any) => ({
      label: t("timelineReturnRequested"),
      at: r.created_at,
    })),
  ]
    .filter((e: any) => e.at)
    ...
    <div className="space-y-6">
        ...
        <tbody>
          {(order.items || []).map((i: any) => (
            <tr key={i.id} className="border-b">
              <td>{i.title}</td>
              <td>{i.quantity}</td>
              <td>{((i.unit_price || 0) / 100).toFixed(2)}</td>
              <td>{((i.total || 0) / 100).toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex gap-2">
        {order.fulfillment_status !== "fulfilled" && (
          <Button onClick={() => setFulfillOpen(true)}>
            {t("createFulfillment")}
          </Button>
        )}
      ...
      <div className="flex gap-2">
        <Button
          variant="secondary"
          onClick={() => router.push(`${window.location.pathname}/returns`)}
        >
          {t("manageReturns")}
        </Button>
      </div>
      ...
      <div className="rounded border p-4">
        <h3 className="text-lg-semi">{t("orderTimeline")}</h3>
      ...
      <div className="rounded border p-4 space-y-2">
        <h3 className="text-lg-semi">{t("orderNotes")}</h3>
        {notes.length === 0 ? (
      ...
      <Modal isOpen={fulfillOpen} close={() => setFulfillOpen(false)}>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setFulfillOpen(false)}>
            {t("cancelAction")}
          </Button>
          <Button
            disabled={itemIds.size === 0 || pending}
            onClick={() =>
              startTransition(async () => {
                const payload = itemsToFulfill
      ...
      <Modal isOpen={refundOpen} close={() => setRefundOpen(false)}>
        ...
        <Modal.Footer>
          ...
          <Button
            disabled={
              pending || amount <= 0 || amount > (order.total || 0) / 100
            }
            onClick={() =>
              startTransition(async () => {
                await createRefund(
